...
namespace PizzaciSon1
{
    public partial class SalonMasalar : Form
    {
        ...
        private void SalonMasalar_Load(object sender, EventArgs e)
        {
            ...
            if (VeriTabanı.veriVarmi("select * from Masalar where masaYeri='Salon' and durum=1"))
            {
                btnMasaSayisi.Text = "Toplam Masa/Dolu Masa: " + urunAdet + "/" + doluMasa.ToString();
                try
                {
                    bg.Open();
                    SqlCommand cmd2 = new SqlCommand("select sum(fiyat) as toplamUcret from Masalar where durum=1 and masaYeri='Salon'", bg);
