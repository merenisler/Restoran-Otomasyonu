...
namespace PizzaciSon1.Ayarlar
{
    public partial class MasaEkleKaldir : Form
    {
        ...
        private void btnKaldir_Click(object sender, EventArgs e)
        {
            ...
            string masaYeriSecim = "";
            lstViewMasalar.Items.Clear();
            masaYeriSecim = cmbBxMasaYeri2.Text;
            bg.Open();
            cmd = new SqlCommand("select * from Masalar where masaYeri='" + masaYeriSecim + "' order by masaNo", bg);
            SqlDataReader oku = cmd.ExecuteReader();
            while (oku.Read())
            {
                ListViewItem listele = new ListViewItem();
        ...
        private void btnEkle_Click(object sender, EventArgs e)
        {
            ...
            if (yeniMasaYeri != "" && yeniMasaNo != 0)
            {
                ...
                bg.Open();
                guncel = new SqlCommand("create table " + masa + "(yazdir bit, adet int, isim nvarchar(50), fiyat int, stokKategori nvarchar(30), kategori nvarchar(10));", bg);
                guncel.ExecuteNonQuery();
                bg.Close();
