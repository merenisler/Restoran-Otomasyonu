...
namespace PizzaciSon1.Ayarlar
{
    public partial class AyarlarGiris : Form
    {
        ...
        private bool GirisKontrol(int password)
        {
            ...
            {
                connection.Open();
                using (SqlCommand command = new SqlCommand("SELECT COUNT(*) FROM Guvenlik WHERE sifre=@sifre and pozisyon='Patron'", connection))
                {
        ...
        private void btnGiris_Click(object sender, EventArgs e)
        {
            ...
            if (GirisKontrol(girilenSifre))
            {
                ...
                bg.Open();
                SqlCommand cmd = new SqlCommand("select * from Guvenlik where sifre=" + girilenSifre + " and pozisyon='Patron'", bg);
                try
                {
